...
MASAS = {
    ...
    "Masa Brioche (EXCLUSIVA ROSCA)": {
        ...
        "procedimiento": ["Tangzhong 1:1", "Reposo en bloque extendido 12h a 4°C", "T° final: 23-24°C"]
    },
    ...
    "Brownie (Batch 20x20)": {
        ...
        "merma": 1.0,
        "tangzhong": None,
        "procedimiento": ["Avellanar mantequilla", "Mezclar con chocolate caliente", "Añadir azúcares vigorosamente", "Huevos sin montar"]
    }
}
...
EXTRAS = {
    ...
    "Lágrima Concha Vainilla": {"unitario": 30, "receta": {"Harina": 100, "Azúcar Glass": 100, "Mantequilla": 100}},
    "Lágrima Concha Chocolate": {"unitario": 30, "receta": {"Harina": 87.5, "Cacao": 12.5, "Azúcar Glass": 100, "Mantequilla": 100}},
    "Crema Pastelera Vainilla (Berlina)": {"unitario": 80, "receta": {"Leche": 500, "Yemas": 100, "Azúcar": 120, "Fécula": 45, "Mantequilla": 30, "Vainilla": 6}},
    ...
    "Crema Pastelera Choco-Leche Turín": {"unitario": 80, "receta": {"Leche": 450, "Yemas": 100, "Azúcar": 90, "Fécula": 45, "Chocolate Turín Leche": 120, "Mantequilla": 20}},
    ...
    "Glaseado Clásico (Roles)": {"unitario": 10, "receta": {"Azúcar glass": 200, "Leche": 30, "Vainilla": 5, "Sal fina": 0.5}},
...
with t1:
    st.subheader("Añadir al horno de hoy")
    ...
    with c4: r_sel = st.selectbox("Relleno/Extra", list(EXTRAS.keys()))
    with c5: a_sel = st.selectbox("Acabado/Lágrima", list(EXTRAS.keys()))
...
if st.session_state.plan:
    with t2:
        for item in st.session_state.plan:
            ...
            if "fijo" in m: # Lógica para Brownies
                st.subheader(f"Batido para {item['cant']} unidades de molde")
                ...
                sum_p = sum(m['ingredientes'].values())
                h_base = (masa_total * 100) / sum_p
                ...
                with col2:
                    ...
                    if "huesos_reforzados" in m:
                        reserva = masa_total * m['huesos_reforzados']['reserva_masa_porc']
                        h_ex = reserva * m['huesos_reforzados']['harina_extra_porc']
                        y_ex = reserva * m['huesos_reforzados']['yema_extra_porc']
                        st.info(f"🦴 Huesos: Reforzar {reserva:,.1f}g de masa con {h_ex:,.1f}g harina y {y_ex:,.1f}g yema.")
            ...
            for ex_name in [item['extra1'], item['extra2']]:
                if ex_name != "Ninguno":
                    ...
                    peso_ex_total = ex['unitario'] * item['cant']
                    # Si es una receta escalable (como lágrima o crema)
                    if ex['receta']:
                        sum_ex = sum(ex['receta'].values())
                        factor = peso_ex_total / sum_ex if sum_ex > 0 else 1
                        for ing, val in ex['receta'].items():
                            p = val * (factor if "Rebozado" not in ex_name else item['cant'])
                            st.write(f"• {ing}: {p:,.1f}g")
                            compras[ing] = compras.get(ing, 0) + p
            st.divider()

    with t3:
        st.header("🛒 Lista Maestra de Insumos (Totales Sumados)")
